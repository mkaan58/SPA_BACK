...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    def website_limit(self):
        """Kullanıcının website oluşturma limitini döndürür"""
        limits = {
            'free': 2,
            'basic': 5,
            'premium': 20
    ...
    @property
    def websites_remaining(self):
        """Kullanıcının kalan website hakkını döndürür"""
        current_count = self.websites.count()
        return max(0, self.website_limit - current_count)

    @property
    def can_create_website(self):
        """Kullanıcının yeni website oluşturup oluşturamayacağını döndürür"""
        return self.websites_remaining > 0